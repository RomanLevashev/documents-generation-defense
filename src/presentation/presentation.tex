...
\begin{frame}[fragile]
  \frametitle{Введение}
  \begin{itemize}
    \item \textbf{PracticeGrading} --- веб-приложение для подготовки заседаний комиссии и проведения защит ВКР и практик
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Универсальные заполнители шаблонов (Form Pilot и аналоги)
      \begin{itemize}
        \item \textit{Минусы:} данные часто приходится вводить вручную для каждого документа
      \end{itemize}
...
\begin{frame}
  \frametitle{Расширение модели данных и веб-интерфейса\\для управления членами комиссии}
  \begin{itemize}
    \item Расширена модель данных для членов комиссии:
      \begin{itemize}
        \item добавлены новые поля в таблицу \texttt{Users} для хранения сведений,
        необходимых при заполнении документов;
        \item изменена связь между \texttt{Users} и \texttt{Meetings}:
        вместо связи «один ко многим» теперь используется
        связь «многие ко многим» (член комиссии может участвовать
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Добавлены проверки:
      \begin{itemize}
        \item e2e-тесты Playwright: покрыта новая функциональность по добавлению и управлению членами комиссии
...
\begin{frame}
  ...
  \begin{itemize}
    \item Расширена модель данных: добавлены сведения о членах комиссии,
          необходимые для генерации документов.
    \item Реализован веб-интерфейс для управления членами комиссии.
    \item Доработана генерация документов:
      \begin{itemize}
        \item обновлена генерация ранее формируемых документов под новый стандарт:
              \emph{ведомость}, \emph{оценочные листы членов ГЭК};
        \item реализована генерация новых документов:
          ...
          \end{itemize}
        \item весь пакет документов, необходимый для проведения заседания,
              теперь генерируется автоматически на основе данных системы.
      \end{itemize}
  \end{itemize}

  \vfill
  \centering\footnotesize Исходный код реализации: \url{https://github.com/yurii-litvinov/PracticeGrading/pull/11}
\end{frame}
\end{document}
