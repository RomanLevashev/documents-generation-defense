...
### 1.2. Изменения генерации документов для заседаний

Файл: `PracticeGrading.API/Endpoints/MeetingEndpoints.cs`.

#### Уточнение выдачи данных о членах комиссии при просмотре заседания в зависимости от роли
...
подставляя данные заседания, членов комиссии и студенческих работ. Для работы с DOCX используется библиотека **NPOI** (`XWPFDocument`).
...
- `chairman: MemberDto` — председатель;
...
Во всех методах параметр `stream` — это **Stream с DOCX-шаблоном соответствующего документа**.

1) **Ведомость** — файл: `Ведомость ВКР ГЭК {commissionNumber}.docx`  
   Метод: `GenerateStatement(string coordinator, Stream stream)`  
   Где `coordinator` — **ФИО координатора**.

2) **Оценочный лист** — файл: `{member.Name} оценочный лист ГЭК {commissionNumber}.docx`  
   Метод: `GenerateGradingSheet(MemberDto member, Stream stream)`  
   Где `member` — **соответствующий член комиссии**, для которого генерируется оценочный лист.

3) **Согласие на обработку персональных данных** — файл: `{member.Name} Согласие на обработку персональных данных.docx`  
   Метод: `GenerateAgreement(MemberDto member, Stream stream)`  
   Где `member` — **соответствующий член комиссии**, для которого генерируется согласие.

4) **Отчёт председателя** — файл: `Отчёт председателя ГЭК {commissionNumber}.docx`  
   Метод: `GenerateChairmanReport(Stream stream)`

5) **Протокол защиты** — файл: `Протокол защиты {work.StudentName}.docx`  
   Метод: `GenerateDefenseProtocol(StudentWorkDto work, Stream stream)`  
   Где `work` — **студенческая работа**, для которой генерируется протокол защиты.

6) **Итоговый протокол** — файл: `Протокол итоговый {commissionNumber}.docx`  
   Метод: `GenerateFinalProtocol(Stream stream)`
...
Верхнеуровневая сборка комплекта документов для заседания выполняется вне `DocumentsGenerator` (в endpoint’ах),
...
Задача изменений: добавить на UI функциональность для управления членами комиссии и привязки членов комиссии к заседаниям, а также собрать недостающие данные для генерации документов.
...
- интеграция со страницами заседаний: `MeetingFormPage.tsx`, `ViewMeetingPage.tsx`.
...
- `hasMore: boolean` — есть ли ещё члены комиссии;
...
### 2.8 Интеграция со страницами заседаний

### `MeetingFormPage.tsx`
На странице редактирования/создания заседания прежний список членов комиссии заменён на переиспользуемый компонент `MemberList`

### `ViewMeetingPage.tsx`
На странице просмотра заседания:
- добавлен поиск членов комиссии с выпадающим списком через `MemberSearchDropdown` (для добавления в состав заседания);
...
Суть изменения:
- связь **заседание ↔ члены комиссии** переведена на **many-to-many**, чтобы один и тот же член комиссии мог участвовать в нескольких заседаниях;
...
- Endpoint-тесты:
  ...
  - `PracticeGrading.Tests/EndpointsTests/MeetingEndpointsTests.cs` — адаптация под изменения.
...
- **обновлён** сценарий `create meeting`: тест изменён с учётом нового механизма выбора члена комиссии (поиск и выбор из выпадающего списка на странице заседания);
